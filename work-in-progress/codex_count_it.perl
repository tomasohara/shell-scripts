...
sub analyze_pattern_anchors {
    my ($raw_pattern) = @_;
    my ($in_class, $escaped) = (&FALSE, &FALSE);
    my (@anchor_positions) = ();
    my ($len) = length($raw_pattern);

    for (my $i = 0; $i < $len; $i++) {
        my ($ch) = substr($raw_pattern, $i, 1);
        if ($escaped) {
            $escaped = &FALSE;
            next;
        }
        ...
        if (!$in_class) {
            ...
            if ($ch eq "]") {
                # Keep POSIX char-class delimiters (e.g., [[:alpha:]]) inside class.
                my ($prev) = (($i > 0) ? substr($raw_pattern, $i - 1, 1) : "");
                my ($next) = (($i + 1 < $len) ? substr($raw_pattern, $i + 1, 1) : "");
        ...
        if (($ch eq "^") || ($ch eq "\$")) {
            push(@anchor_positions, $i);
        }
    }

    my ($has_start_anchor, $has_end_anchor, $has_internal_anchor) = (&FALSE, &FALSE, &FALSE);
    my ($last_index) = ($len - 1);
    foreach my $position (@anchor_positions) {
        if ($position == 0) {
            $has_start_anchor = &TRUE;
        }
        elsif ($position == $last_index) {
            $has_end_anchor = &TRUE;
        }
        else {
            $has_internal_anchor = &TRUE;
        }
    }
    return (($has_start_anchor || $has_end_anchor), $has_internal_anchor);
}
my($has_line_anchor, $has_internal_anchor) = &analyze_pattern_anchors($pattern);
...
if ((! $one_per_line) && (! $has_line_anchor) && $has_internal_anchor) {
