...
my($starts_with_caret) = (($pattern =~ /^\^/) ? 1 : 0);
my($ends_with_dollar) = (($pattern =~ /[\$\n]$/) ? 1 : 0);
my($has_line_anchor) = ($starts_with_caret || $ends_with_dollar);
...
my($has_internal_anchor) = (($pattern =~ /(?<!\\)(?<!\[)[\^\$]/) ? 1 : 0);
...
if ((&DEBUG_LEVEL > 3) && $starts_with_caret && (! $ends_with_dollar) && (! $multi_per_line)) {
