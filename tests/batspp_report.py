...
#
# TODO1:
# - Track down cygwin problem with sh as symbolic link to bash: the workaround
#   is to link it to dash (as under Ubuntu).
#
# Note:
...
def main():
    ...
    ## OLD: check_errors_script = gh.resolve_path("../check_errors.perl"))
    check_errors_script = gh.form_path(parent_dir, "check_errors.perl")
    ...
    for testfile in ipynb_array:
        ...
        if is_ipynb:
            ...
            ## OLD: gh.run(f"python3 ../jupyter_to_batspp.py {extra_args} --output {batspp_from_ipynb} {testfile} 2> {log_file}")
            gh.run(f"python3 {parent_dir}/jupyter_to_batspp.py {extra_args} --output {batspp_from_ipynb} {testfile} 2> {log_file}")
    ...
    if TXT_OPTION:
        ...
        def print_test_array(arr):
            ...
            for index, item in enumerate(arr):
                ...
                ## BAD: print(f"{index+1}. {t_name} ({t_rate}%; {t_count_ok}/{t_count_total} OK): threshold={min_score}%; {t_count_eval_error} evaluation errors")
