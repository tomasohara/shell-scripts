#!/bin/bash
#
# Adhoc testing script developed by Aviyan for running the tests with coverage checks
# enabled.
#
# Warning: the script is work in progress.
#
...
# show_usage(): displays usage statment and then exits
function show_usage {
    script="${BASH_SOURCE[0]}"
    echo "Usage: $script [--source alias-script] [--help] [-]"
    echo ""
    echo "Example:"
    echo "    TEST_REGEX='.*alias-grep-commands.*' $0 -"
    echo ""
    echo "Note:"
    echo "- Env options: RL, TEST_REGEX, TMP, SOURCE_SCRIPT"
    echo "- Use - to avoid usage statement."
    echo "- Default output location: $TMP/batspp/out"
    # TODO3: mention default settings (e.g, test_regex)
    exit
}
...
if [[ -n "$SOURCE_SCRIPT" ]]; then
    ...
    # Source the script
    # TODO3: make sure batspp doesn't override the aliases
