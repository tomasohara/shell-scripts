...
class KcovTestCoverage:
    ...
    def __init__ (
        ...
        self.facts_array = None
        self.report_InsLine = None
        self.report_CovLine = None
        self.report_RatioLine = None
        self.report_JSON = None
        self.facts_JSON = None
        self.ratio_array = None
        self.instrumented_array = None
        self.covered_array = None
        self.file_quantity = None
        debug.trace_object(5, self, label=f"{self.__class__.__name__} instance after __init__")
    ...
    def extract_report(self, report_name, i):
        ...
        self.report_JSON = json.loads(json_extract)           
        report_Date = self.report_JSON["date"]
        self.report_InsLine = self.report_JSON["instrumented"]
        self.report_CovLine = self.report_JSON["covered"]
        self.report_RatioLine = round(self.report_CovLine / self.report_InsLine, 4)
...
------------------------------------
Date: {report_Date}
Instrumented Lines: {self.report_InsLine}
Covered Lines: {self.report_CovLine}
Coverage Ratio: {round(self.report_RatioLine, 4)}
"""     
        # facts_JSON is used for determining highest and lowest facts
        self.facts_JSON = {}
        self.facts_JSON["report_name"] = report_name
        self.facts_JSON["instrumented_line"] = self.report_InsLine
        self.facts_JSON["covered_line"] = self.report_CovLine
        self.facts_JSON["ratio"] = self.report_RatioLine
        self.facts_array.append(self.facts_JSON)
        ...
        if self.EXPORT_OPT:
            ...
            # facts_JSON_obj = json.load(self.facts_JSON)
            # pretty_facts_JSON = json.dumps(facts_JSON_obj)

            # [OLD]: JSON output is replaced by simple output
            # gh.write_file(filename=f"{COVERAGE_EXPORT_PATH}/{report_name}_REPORT.txt", text=str(self.facts_JSON))
            if not coverage_folder_exists:
                gh.create_directory(COVERAGE_EXPORT_PATH)

            export_summary = f"{self.report_CovLine} out of {self.report_InsLine} tests passed overall ({self.report_RatioLine*100}%)"
    ...
    def report_process(self):
        ...
        for report in reports:
            ...
            self.ratio_array += [self.report_RatioLine]
            self.instrumented_array += [self.report_InsLine]
            self.covered_array += [self.report_CovLine]
            i += 1

        self.file_quantity = i - 1
    ...
    def summary_stats(self):
        ...
        debug.trace_object(5, self, label=f"{self.__class__.__name__} instance before summary_stats")

        RATIO_MEAN = round(np.mean(self.ratio_array), 4)
        RATIO_MEDIAN = round(np.median(self.ratio_array), 4)

        INSTRUMENTED_TOTAL = np.sum(self.instrumented_array)
        COVERED_TOTAL = np.sum(self.covered_array)

        INSTRUMENTED_MAX = np.max(self.instrumented_array)
        INSTRUMENTED_MIN = np.min(self.instrumented_array)

        HIGHEST_RATIO = np.max(self.ratio_array)
        LOWEST_RATIO = round (np.min(self.ratio_array), 4)

        for fact in self.facts_array:
        ...
        print (f"\nREPORTS DETECTED: [{self.file_quantity}]")
