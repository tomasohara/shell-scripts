...
import shutil  # <-- FIX: This is required for robust directory cleanup.
...
class TestFailureAnalyzer(TestWrapper):
    ...
    # CRITICAL FIX: Add a setUp method to ensure a clean temporary directory
    # is created before EACH test, preventing state leakage between them.
    def setUp(self):
    ...
    def test_analyze_test_run_happy_path(self):
        """
        Tests the analyze_test_run function for the standard success case.
...
""")
        # Run Test
        sorted_failures = THE_MODULE.analyze_test_run(self.temp_base)
    ...
    def test_analyze_test_run_no_output_files(self):
        """
        Tests the analyze_test_run function's behavior when no
    ...
    def test_analyze_test_run_aggregation(self):
        """
        Tests the aggregation logic of analyze_test_run.
...
not ok 3 - another failure # with a trailing comment
        ...
        # Run Test
        failed_tests = THE_MODULE._find_failed_tests(Path(log_file))
...
"""
        ...
        # Run Test
        extracted_command = THE_MODULE._extract_from_generated_script(Path(script_file), "my real test")
        ...
        extracted_command_none = THE_MODULE._extract_from_generated_script(Path(script_file), "non_existent_test")
