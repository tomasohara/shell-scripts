...
class AutomateIPYNB(Main):
    ...
    def return_ipynb_url_array(self):
        ...
        # ipynb_files filtering according to ENV_OPTIONS and argparsing
        # A) SELECT_BATSPP selects each and every file
        ipynb_files = ipynb_files_all.copy() if not SELECT_NOBATSPP else ipynb_files_filtered.copy()

        # B) RANDOMIZE_TESTS randomizes the order of tests
        if RANDOMIZE_TESTS:
            random.shuffle(ipynb_files)  # Shuffle the ipynb_files list if RANDOMIZE_TESTS is True
        ...
        # D) --include option
        # ipynb_files = [INCLUDED_FILE] if --include True else ipynb_files
        # E) --first_n_testfiles option
        # ipynb_files = ipynb_files(range_to_n) IF --first True else ipynb_files
