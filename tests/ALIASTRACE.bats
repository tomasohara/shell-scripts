...
function test6-assert2-expected () {
	echo -e '1991  ## You will need to run jupyter from that directory.1992  alias | wc -l1993  echo $?1994  ## NOTE: For reproducability, the directory name needs to be fixed1995  ## In place of $$, use a psuedo random number (e,g., 1210)1996  ## *** All output from one run to the next needs to be the same ***1997  ## temp_dir=$TMP/test-$$1998  temp_dir=$TMP/test-12101999  mkdir -p "$temp_dir"2000  # TODO: /bin/rm -rvf "$temp_dir"2001  cd "$temp_dir"2002  pwd2003  #ALIAS FOR PRINTING SEPERATION LINES (FOR JUPYTER)2004  alias linebr="printf \'%*s\\n\' "${COLUMNS:-$(tput cols)}" \'\' | tr \' \' -"2005  echo $?2006  # Count aliases proper2007  alias | wc -l2008  echo $?2009  # Count functions2010  typeset -f | egrep \'^\\w+\' | wc -l2011  echo $?2012  ##1 SHOWS HISTORY OF BASH COMMANDS2013  alias hist=\'history $LINES\'2014  hist'
}
...
function test7-assert2-expected () {
	echo -e '1999  mkdir -p "$temp_dir"2000  # TODO: /bin/rm -rvf "$temp_dir"2001  cd "$temp_dir"2002  pwd2003  #ALIAS FOR PRINTING SEPERATION LINES (FOR JUPYTER)2004  alias linebr="printf \'%*s\\n\' "${COLUMNS:-$(tput cols)}" \'\' | tr \' \' -"2005  echo $?2006  # Count aliases proper2007  alias | wc -l2008  echo $?2009  # Count functions2010  typeset -f | egrep \'^\\w+\' | wc -l2011  echo $?2012  ##1 SHOWS HISTORY OF BASH COMMANDS2013  alias hist=\'history $LINES\'2014  hist2015  echo $?2016  ##2 SHOWS HISTORY WITHOUT TIMESTAMPS2017  # Removes timestamp from history (e.g., " 1972  [2014-05-02 14:34:12] dir *py" => " 1972  dir *py")2018  # TEST: function hist { h | perl -pe \'s/^(\\s*\\d+\\s*)(\\[[^\\]]+\\])(.*)/$1$3/;\'; }2019  # note: funciton used to simplify specification of quotes2020  # quiet-unalias h #COMMAND NOT FOUND2021  function h { hist | perl -pe \'s/^(\\s*\\d+\\s*)(\\[[^\\]]+\\])(.*)/$1$3/;\'; }2022  h'
}
...
function test8-assert2-expected () {
	echo -e 'PID TTY          TIME CMD35020 pts/5    00:00:00 bash35048 pts/5    00:00:00 ps35049 pts/5    00:00:00 bash35050 pts/5    00:00:00 perl'
}
...
@test "test27" {
	...
	dobackup.sh process1.txt > /dev/null #THROWS UNEXPECTED EOF ERROR ON OUTPUT
}
