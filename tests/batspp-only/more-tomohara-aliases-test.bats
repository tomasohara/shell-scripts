...
@test "test-1" {
	testfolder="/tmp/batspp-95327/test-1"
...
function test-3-setup () {
	# no-op in case content just a comment
	true

	testfolder="/tmp/batspp-95327/test-3"
	mkdir --parents "$testfolder"
	command cp ./*.* "$testfolder"
	builtin cd "$testfolder" || echo Warning: Unable to "cd $testfolder"
...
@test "test-4" {
	testfolder="/tmp/batspp-95327/test-4"
	mkdir --parents "$testfolder"
	command cp ./*.* "$testfolder"
	builtin cd "$testfolder" || echo Warning: Unable to "cd $testfolder"
...
@test "test-5" {
	testfolder="/tmp/batspp-95327/test-5"
	mkdir --parents "$testfolder"
	command cp ./*.* "$testfolder"
	builtin cd "$testfolder" || echo Warning: Unable to "cd $testfolder"
...
@test "test-6" {
	testfolder="/tmp/batspp-95327/test-6"
	mkdir --parents "$testfolder"
	command cp ./*.* "$testfolder"
	builtin cd "$testfolder" || echo Warning: Unable to "cd $testfolder"
