...
eval 'exec perl -Ssw $0 "$@"'
    if 0;
# TODO: add '*-*-perl-*-*' back to magic line
# OLD" eval 'exec perl -Ssw $0 "$@"'
...
use vars qw/$kill $status $remote $no_files $pause $all $busy_load $trace
            $quote $noquote $nonfile $from $to $step $count $DOMAINNAME $HOST $hostlist $f $F $h $b $B $d/;
...
# $b, &F, &B &h, not given).
if (($command !~ /[\$\&]\{?[fbdBh]\}?/) && ($no_files == &FALSE)) {
    $command .= " \$f";
...
$script_name 'bash -i -c \"spanish \$f\"' perro gato
