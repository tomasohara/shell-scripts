...
class CountIt(Main):
    ...
    def setup(self):
        ...
        self.ignore_case = (
            self.has_parsed_option(IGNORE_SHORT)
            or self.has_parsed_option(IGNORE_LONG)
        )
        foldcase = (
            self.has_parsed_option(FOLD)
            or self.has_parsed_option(FOLDCASE)
            or self.ignore_case
        )
        self.preserve = (
            self.has_parsed_option(PRESERVE)
            or not foldcase
        )
        ...
        nonsingletons = (
            self.has_parsed_option(MIN2)
            or self.has_parsed_option(MULTIPLE)
            or self.has_parsed_option(NONSINGLETON)
            or self.has_parsed_option(NONSINGLETONS)
        )
...
if __name__ == '__main__':
    app = CountIt(
        description = __doc__,
        boolean_options = [
            (IGNORE_SHORT, 'ignore case in the pattern matching'),
        ...
        positional_arguments = [
            (PATTERN, 'regex pattern or file with pattern (to circumvent shell UTF8 issues) to check for'),
        ],
        int_options = [
            (OCCURRENCE_FIELD, 'field giving occurrence count'),
            (MIN_FREQ, 'min frequency to show in output'),
        ],
        text_options = [
            (RESTORE, 'simulate look-ahead'),
        ],
    )
    app.run()
