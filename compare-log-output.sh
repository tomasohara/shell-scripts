...
diff=kdiff.sh
## ignore=""
## TODO: ignore='(\b|0x)[0-9A-F]{8}\b'
##
## OLD: ignore='(0x)?[0-9A-Fa-f]{7,8}'
ignore='(0x)?[0-9A-Fa-f]{7,8}'
...
if [ "$1" = "" ]; then
    ...
    echo ""
    echo "Usage: `basename $0` [options] file1 file2-or-dir"
    echo ""
    echo "    options: [--ignore perl-regex] [--included-ptrs] [--plain-diff] [--diff program]"
    echo ""
    echo "Examples:"
    echo ""
    ...
    echo ""
    echo "Notes:"
    echo "- The regex pattern matching is noot case sensitive (i.g., case ignored)."
    echo "- Timestamps are removed prior to comparison."
    echo "- Also hex addresses are ignored as with '--ignore [0-9A-Fa-f]{8}'."
    echo "- Additional filtering can be done via --ignore, but only one can be given (implies --included-ptrs)."
...
# NOTE: Backslaahes used above to avoid shell interpretation (during string interpolation).
